...
class ModelInterface():
    ...
    ModelInterface
    """
    def __init__(self, model, epoch = 10, \
            batch_size = 64, sw_name = 'interface_test'):
        """
        __init__
        """
        self.epoch = epoch
        ...
        self.optimizer = torch.optim.SGD(model.fc.parameters(), \
    ...
    def load_data_template(self):
        ...
        train_data = datasets.MNIST(root = os.getcwd(), \
                train = False, transform = transform, download = False)
        test_data = datasets.MNIST(root = os.getcwd(), \
                train = False, transform = transform, download = False)
    ...
    def train_template(self):
        ...
        for self.index in range(1, self.epoch + 1):
            ...
            writer.add_scalar("Train/Loss", \
                    loss.item(), self.index)
            print('epoch{} loss:{:.4f} cost:{}'.format( \
    ...
    def save_model_template(self, fname = ''):
        ...
        """
        if fname == '':
            fname = './models/e-{}_ac-{:.2f}.pt' % \
                    (self.index, self.accuracy)
